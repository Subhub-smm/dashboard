...
// SMM Panel MVP (Node.js + Express Backend) with Payment Gateway and Admin Dashboard
...
app.post('/register', async (req, res) => {
  ...
  db.query('INSERT INTO users (username, password, balance) VALUES (?, ?, 0)', [username, hashedPassword], (err, result) => {
...
app.post('/login', (req, res) => {
  ...
  db.query('SELECT * FROM users WHERE username = ?', [username], async (err, results) => {
    ...
    const token = jwt.sign({ userId: user.id, username: user.username, isAdmin: user.username === 'admin' }, 'secretKey');
    res.json({ token });
  });
});
...
app.post('/admin/topup', (req, res) => {
  ...
  if (adminKey !== 'Haffkast@2004') return res.status(403).json({ message: 'Forbidden' });

  db.query('UPDATE users SET balance = balance + ? WHERE username = ?', [amount, username], (err, result) => {
